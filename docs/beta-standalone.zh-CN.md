...
- 可选的身份、用户画像与关系归位允许明确把候选送入 self-core、用户画像或关系网，默认关闭；
- 重复、补充、更迭、冲突与仅相关采用不同整合结构；
- 修订保留旧版本，替换保留事实变化链；
- 通用个人空间可维护带来源和版本的 self-core、独立用户画像与关系节点，但发布包不包含任何真实人物资料；
- 分层召回分别限制 self-core、用户画像、关系、近期、长期与历史层的上下文预算；
- 候选可获得零写入的近期、长期或不确定建议；确认后的近期记忆必须带失效时间，到期后退出普通召回；
- 整库 JSON 导出与导入；
- 手机优先的本地前端；
...
MCP 向记忆库所属的 Agent 开放与人类工作台等价的能力：总览、搜索与读取、候选治理、整合预览与执行、赋权、修订、替换、归档恢复、审计、日历、self-core、用户画像、关系网、分层召回、审阅模式及整库导入导出。Moraine 后端本身没有永久删除接口，因此 MCP 也不伪造一个不可恢复的删除能力。
...
Moraine 提供的是 `POST /api/wakeup/preview` 安全预览，不是完整的自动唤醒运行时。它不会自己定时醒来，也不包含轮换器、消息发送器或行动执行器。部署者需要自行准备宿主 Agent 或编排器，完成以下链路：

1. 用 systemd timer、cron 或宿主平台的调度能力决定何时唤醒；
2. 收集经过最小化的真实信号，并调用唤醒预览接口；
3. 由宿主 Agent 判断保持安静、选择某项行动或换方向；
4. 仅在宿主自身的权限边界内执行，并单独记录真实结果。

不配置 Jev 也可以完成上述链路。Jev 只是编排器可选择调用的第二意见，不能读取记忆正文、身份文字、关系备注或凭证，也不能执行、外发或写记忆。`can_execute`、`can_write_memory` 与 `excluded` 是交给宿主的能力契约；接入方必须实际校验这些字段，不能把它们当作展示文案。
...
候选箱会在存在可核验依据时显示“建议近期”或“建议长期”。建议只依据明确类型、失效时间，以及现有事件篮子、来源和审计记录可零写入推算的跨时观察与复核证据；不读取正文猜测关系或人物意图，也不靠普通召回次数自我强化。证据不足时仍允许审阅者手动选择，但前端不重复展示“稳定性不足”噪音。

确认入库时可以选择近期或长期。近期记忆必须设置失效时间，到期后退出普通分层召回；长期记忆不设置 TTL。身份、关系与边界候选不会进入普通分层确认，仍需走 self-core、用户画像或关系网的专门审阅入口。
